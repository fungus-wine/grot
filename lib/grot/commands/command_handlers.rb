  ...
  module Commands
    ...
    module Handlers
      ...
      def self.validate_command(app, config)
        ...
        split = config.dig(:giga_options, :split)
        if split && !%w[100_0 75_25 50_50].include?(split)
          errors << "giga_options.split must be one of '100_0', '75_25', '50_50' (got '#{split}')"
        end

        target_core = config.dig(:giga_options, :target_core)
        if target_core && !%w[cm4 cm7].include?(target_core)
